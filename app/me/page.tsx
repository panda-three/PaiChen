...
export default async function CustomerCenter({ searchParams }: { searchParams: Promise<{ store?: string; ref?: string }> }) {
  ...
  if (appUser && (appUser.role === Role.EMPLOYEE || appUser.role === Role.STORE_ADMIN) && appUser.store) {
    const orders = await db.order.findMany({ where: { appSubmitterId: appUser.id }, include: { store: true, items: true }, orderBy: { createdAt: "desc" } });
    return <StaffCenter user={{ ...appUser, store: appUser.store }} orders={orders}/>;
  }
  ...
  const [profiles, orders] = await Promise.all([
    ...
    db.order.findMany({ where: { appSubmitterId: actor.id, ...(selectedStore ? { storeId: selectedStore.id } : {}) }, include: { store: true, items: true }, orderBy: { createdAt: "desc" } }),
  ]);
  ...
  return <div className="public-desktop"><main className="public-phone public-me"><header>{currentProfile?.avatarUrl?<img className="public-avatar-image" src={currentProfile.avatarUrl} alt="个人头像"/>:<div className="public-avatar"><UserRound/></div>}<div><h1>{currentProfile?.name ?? actor.name}</h1><p>登录手机号 {actor.phone}</p></div></header><nav className="public-me-summary"><a href="#orders"><PackageCheck/><b>{orders.length}</b><span>我的订单</span></a><Link href={`/me/settings${settingsParams.size ? `?${settingsParams}` : ""}`}><Settings/><b>账号</b><span>设置</span></Link></nav><section><h2><StoreIcon/> 我的店铺</h2>{profiles.map((item) => <Link className="public-profile" href={`/s/${item.store.slug}${query.ref ? `?ref=${encodeURIComponent(query.ref)}` : ""}`} key={item.id}><span>{item.store.name}</span><small>{item.status === "ACTIVE" ? "已激活" : item.status}</small></Link>)}</section><section id="orders"><h2>我的订单</h2>{orders.map((order) => <article className="public-order" key={order.id}><header><b>{order.store.name}</b><span>{order.status}</span></header><p>{order.items.map((item) => `${item.productName} × ${item.quantity}`).join("、")}</p><small>{order.orderNo} · {formatDate(order.createdAt)}</small></article>)}{!orders.length && <div className="public-empty">暂无订单</div>}</section><Link className="public-back-store" href={back}>返回店铺</Link></main></div>;
}
