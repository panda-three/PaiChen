...
export default async function CustomerSettingsPage({ searchParams }: { searchParams: Promise<{ store?: string; ref?: string }> }) {
  ...
  return <CustomerSettings loginPhone={actor.phone ?? actor.username} profile={{ id: profile.id, storeSlug: profile.store.slug, storeName: profile.store.name, name: profile.name, phone: profile.phone, avatarUrl: profile.avatarUrl, servicePhone: profile.servicePhone, serviceWechat: profile.serviceWechat, serviceQrUrl: profile.serviceQrUrl, cardTitle: profile.cardTitle, cardBio: profile.cardBio }} refCode={query.ref ?? ""}/>;
}
