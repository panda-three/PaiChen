...
import { Camera, ChevronLeft, LockKeyhole, LogOut, QrCode, ShieldCheck, Smartphone, UserRound } from "lucide-react";
import { useEffect, useState } from "react";

type Profile = {
  id: string; storeSlug: string; storeName: string; name: string; phone: string;
  avatarUrl: string | null; servicePhone: string | null; serviceWechat: string | null;
  serviceQrUrl: string | null; cardTitle: string | null; cardBio: string | null;
};
...
export function CustomerSettings({ loginPhone, profile: initialProfile, refCode }: { loginPhone: string; profile: Profile; refCode: string }) {
  ...
  async function saveProfile(event: React.FormEvent<HTMLFormElement>) {
    ...
    const response = await fetch("/api/customer/settings/profile", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
      storeSlug: profile.storeSlug, name: data.get("name"), phone: data.get("phone"), servicePhone: data.get("servicePhone"), serviceWechat: data.get("serviceWechat"), cardTitle: data.get("cardTitle"), cardBio: data.get("cardBio"), currentPassword: data.get("currentPassword") || undefined,
    }) });
  ...
  async function upload(type: "avatar" | "serviceQr", file: File | undefined) {
    if (!file) return; setBusy(true); setMessage("");
    const body = new FormData(); body.set("storeSlug", profile.storeSlug); body.set("type", type); body.set("file", file);
    const response = await fetch("/api/customer/settings/assets", { method: "POST", body }); const result = await response.json();
    if (response.ok) setProfile((current) => ({ ...current, [type === "avatar" ? "avatarUrl" : "serviceQrUrl"]: result.url }));
    setMessage(response.ok ? "图片已更新" : result.error ?? "上传失败"); setBusy(false);
  }

  async function removeAsset(type: "avatar" | "serviceQr") {
    setBusy(true); const response = await fetch("/api/customer/settings/assets", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ storeSlug: profile.storeSlug, type }) }); const result = await response.json();
    if (response.ok) setProfile((current) => ({ ...current, [type === "avatar" ? "avatarUrl" : "serviceQrUrl"]: null }));
  ...
  return <div className="public-desktop"><main className="public-phone public-settings">
    ...
    <section className="settings-identity"><div className="settings-avatar">{profile.avatarUrl ? <img src={profile.avatarUrl} alt="个人头像"/> : <UserRound/>}<label title="上传头像"><Camera/><input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={(event) => void upload("avatar", event.target.files?.[0])}/></label></div><div><strong>{profile.name}</strong><span>全局登录手机号 {loginPhone}</span>{profile.avatarUrl && <button type="button" disabled={busy} onClick={() => void removeAsset("avatar")}>移除头像</button>}</div></section>
    {message && <p className="settings-message" role="status">{message}</p>}
    <form className="settings-card settings-form" onSubmit={saveProfile}><h2><UserRound/> 个人资料</h2><label>昵称<input name="name" defaultValue={profile.name} maxLength={50} required/></label><label>本店联系电话<input name="phone" defaultValue={profile.phone} pattern="1[0-9]{10}" required/></label><label>客服电话<input name="servicePhone" defaultValue={profile.servicePhone ?? ""} maxLength={30}/></label><label>客服微信<input name="serviceWechat" defaultValue={profile.serviceWechat ?? ""} maxLength={60}/></label><label>名片主标题<input name="cardTitle" defaultValue={profile.cardTitle ?? ""} maxLength={50}/></label><label>名片简介<textarea name="cardBio" defaultValue={profile.cardBio ?? ""} maxLength={200}/></label><label>当前密码 <small>仅在修改本店联系电话时必填</small><input name="currentPassword" type="password" autoComplete="current-password"/></label><button disabled={busy}>保存当前店资料</button></form>
    <section className="settings-card settings-qr"><h2><QrCode/> 客服二维码</h2>{profile.serviceQrUrl ? <img src={profile.serviceQrUrl} alt="客服二维码"/> : <div className="settings-qr-empty">尚未上传</div>}<div><label className="settings-upload">上传或替换<input type="file" accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={(event) => void upload("serviceQr", event.target.files?.[0])}/></label>{profile.serviceQrUrl && <button type="button" disabled={busy} onClick={() => void removeAsset("serviceQr")}>移除</button>}</div></section>
