...
type Product = { id: string; name: string; code: string; mainImageUrl: string; categoryId: string | null };
...
export function GroupCatalog({ slug, refCode, groups, products }: { slug: string; refCode?: string; groups: Group[]; products: Product[] }) {
  ...
  const visible = active ? products.filter((product) => product.categoryId === active.categoryId).slice(0, active.limit ?? undefined) : [];
