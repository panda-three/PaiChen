...
export type Employee = { name: string; phone: string | null; wechat: string | null; wechatQrUrl?: string | null; title: string | null; bio: string | null; avatarUrl: string | null };
type StorePageLink = { id: string; slug: string };

export function PublicHome({ catalog, config, employee, currentPageId, refCode, favoriteIds, pages = [], renderComponent, heroAutoplay = true }: { catalog: HomeCatalog; config: PageConfigV4; employee: Employee; currentPageId?: string; refCode?: string; favoriteIds: string[]; pages?: StorePageLink[]; renderComponent?: (component: PageComponentV4, content: ReactNode) => ReactNode; heroAutoplay?: boolean }) {
  ...
  const bySource = (component: Extract<PageComponentV4, { type: "productGrid" | "newProducts" }>) => catalog.products.filter((product) => component.source.mode === "all" || (component.source.mode === "category" ? categoryProductMatches(component.source.categoryId, product.categoryId, product.category?.parentId ?? null) : component.source.productIds.includes(product.id)));
  ...
  function block(item: PageComponentV4) {
    if (item.type === "heroCarousel") {
      ...
      return <HeroCarousel slides={slides} fallback={fallback} autoplay={heroAutoplay}/>;
    }
    if (item.type === "employeeCard") return <><section className={`public-adviser public-adviser-${item.style}`}><img src={employee.avatarUrl || catalog.store.logoUrl || fallback} alt={employee.name}/><div><strong>{employee.title || "店铺顾问"}</strong><h3>{employee.name}</h3><p>{employee.bio || "为你提供专业选品服务"}</p></div><nav>{employee.phone && <a href={`tel:${employee.phone}`} aria-label={`致电 ${employee.phone}`}><Phone size={17}/><span>电话</span><small className="public-contact-tip" role="tooltip">{employee.phone}</small></a>}{employee.wechat && employee.wechatQrUrl && <button aria-label={`查看${employee.name}的微信名片`} onClick={() => setWechatOpen(true)}><MessageCircle size={17}/><span>微信</span></button>}</nav></section>{wechatOpen && employee.wechat && employee.wechatQrUrl && <WechatSheet name={employee.name} wechat={employee.wechat} qrUrl={employee.wechatQrUrl} onClose={() => setWechatOpen(false)}/>}</>;
    if (item.type === "quickNav") { const iconMap={building:Building2,sofa:Sofa,images:Images,shield:ShieldCheck,phone:PhoneCall}; const pageMap=new Map(pages.map((page)=>[page.id,page.slug])); const entries = item.items.length ? item.items.slice(0, 5) : catalog.categories.filter((category) => !category.parentId).slice(0, 5).map((category) => ({ title: category.name, imageUrl: "", href: storeHref(catalog.store.slug, `category?category=${category.id}`, refCode) })); return <nav className="public-quick">{entries.map((entry, index) => {const Icon="icon" in entry&&entry.icon?iconMap[entry.icon]:ChevronRight;const pageSlug="pageId" in entry&&entry.pageId?pageMap.get(entry.pageId):undefined;const href=pageSlug?storeHref(catalog.store.slug,`p/${pageSlug}`,refCode):entry.href||storeHref(catalog.store.slug,"category",refCode);return <Link href={href} key={index}><span className="public-quick-icon">{entry.imageUrl ? <img src={entry.imageUrl} alt=""/> : <Icon size={20}/>}</span><b>{entry.title}</b></Link>})}</nav>; }
    ...
    if (item.type === "seriesShowcase") { const categories = (item.categoryIds.length ? catalog.categories.filter((category) => item.categoryIds.includes(category.id)) : catalog.categories.filter((category) => !category.parentId).slice(0, 2)); return <section className="public-section"><header><h2>{item.title}</h2></header><div className="public-series">{categories.map((category) => <Link href={storeHref(catalog.store.slug, `category?category=${category.id}`, refCode)} key={category.id}><img src={catalog.products.find((product) => categoryProductMatches(category.id, product.categoryId, product.category?.parentId ?? null))?.mainImageUrl || fallback} alt=""/><span>{category.name}</span></Link>)}</div></section>; }
    ...
    if (item.type === "categoryNav") return <nav className="public-category-pills">{catalog.categories.filter((category) => !category.parentId).map((category) => <Link key={category.id} href={storeHref(catalog.store.slug, `category?category=${category.id}`, refCode)}>{category.name}</Link>)}</nav>;
