...
export default async function PageEdit({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; notice?: string }> }) {
  ...
  if (!page) notFound();
  const [categories, products, publishedPages] = await Promise.all([
    db.category.findMany({ where: { storeId: page.storeId, isActive: true }, orderBy: { sort: "asc" }, select: { id: true, name: true, createdAt: true, _count: { select: { products: { where: { isPublished: true, isDeleted: false } } } } } }),
    db.product.findMany({ where: { storeId: page.storeId, isPublished: true, isDeleted: false, category: { isActive: true }, OR: [{ source: { not: ProductSource.ENTERPRISE } }, { authorization: { status: AuthorizationStatus.ACTIVE } }] }, include: { variants: { orderBy: { sort: "asc" } } }, orderBy: [{ sort: "asc" }, { createdAt: "desc" }] }),
    db.storePage.findMany({ where: { storeId: page.storeId, id: { not: page.id }, publishedAt: { not: null }, publishedJson: { not: null } }, orderBy: { createdAt: "asc" }, select: { id: true, title: true, slug: true } }),
  ]);
  let card = { name: page.store.name, phone: page.store.phone, wechat: null, title: "店铺顾问", bio: page.store.address, avatarUrl: page.store.logoUrl, shareCode: null } as { name: string; phone: string | null; wechat: string | null; title: string | null; bio: string | null; avatarUrl: string | null; shareCode: string | null };
  ...
  return <>
    ...
    <PageEditor
      ...
      categories={categories.map((category) => ({ id: category.id, name: category.name, createdAt: category.createdAt.toISOString(), productCount: category._count.products }))}
