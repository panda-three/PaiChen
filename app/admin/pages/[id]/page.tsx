...
export default async function PageEdit({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <PageEditor page={{ id: page.id, title: page.title, slug: page.slug, config:parsePageConfig(page.draftJson), published: Boolean(page.publishedAt), isHome: page.isHome }} publicUrl={`/s/${page.store.slug}${page.isHome ? "" : `/p/${page.slug}`}`} store={{slug:page.store.slug,name:page.store.name,logoUrl:page.store.logoUrl,phone:page.store.phone,address:page.store.address}} categories={categories} employee={card} products={products.map(product=>({id:product.id,name:product.name,code:product.code,mainImageUrl:product.mainImageUrl,detailImageUrls:product.detailImageUrls,specification:product.specification,price:product.price?.toString()??null,unit:product.unit,description:product.description,categoryId:product.categoryId,variants:product.variants.map(variant=>({id:variant.id,name:variant.name,code:variant.code,price:variant.price?.toString()??null}))}))}/>;
}
