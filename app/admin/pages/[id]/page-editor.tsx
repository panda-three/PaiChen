...
import { buildCategoryBranches, categoryPath, paginateCategoryBranches } from "@/lib/category-tree";
...
type ProductGroup = { categoryId: string; alias?: string; limit: number | null };

function ProductGroupTree({ categories, groups, onChange }: { categories: Category[]; groups: ProductGroup[]; onChange: (groups: ProductGroup[]) => void }) {
  const [query, setQuery] = useState("");
  const [page, setPage] = useState(0);
  const branches = buildCategoryBranches(categories, query);
  const visible = paginateCategoryBranches(branches, page, 6);
  const toggle = (categoryId: string, checked: boolean) => onChange(checked ? [...groups, { categoryId, limit: null }] : groups.filter((group) => group.categoryId !== categoryId));
  return <div className="grid gap-2"><input className="field" placeholder="搜索分组" value={query} onChange={(event) => { setQuery(event.target.value); setPage(0); }}/><div className="grid gap-2">{visible.map(({ root, children }) => <div className="rounded border border-[#e6e1da] p-2" key={root.id}>
    <label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={groups.some((group) => group.categoryId === root.id)} disabled={!groups.some((group) => group.categoryId === root.id) && groups.length >= 15} onChange={(event) => toggle(root.id, event.target.checked)}/><strong>{root.name}</strong><span className="muted ml-auto">{root.productCount}</span></label>
    {children.map((category) => <label className="ml-6 flex items-center gap-2 border-t border-[#eeeae5] py-2 text-sm" key={category.id}><input type="checkbox" checked={groups.some((group) => group.categoryId === category.id)} disabled={!groups.some((group) => group.categoryId === category.id) && groups.length >= 15} onChange={(event) => toggle(category.id, event.target.checked)}/>{category.name}<span className="muted ml-auto">{category.productCount}</span></label>)}
  </div>)}</div><div className="flex justify-between"><button className="btn" type="button" disabled={!page} onClick={() => setPage((value) => value - 1)}>上一页</button><button className="btn" type="button" disabled={(page + 1) * 6 >= branches.length} onClick={() => setPage((value) => value + 1)}>下一页</button></div></div>;
}
...
export function PageEditor({ page, publicUrl, store, employee, products, categories, pages }: Props) {
  ...
  function Properties() {
    if (showGroupPicker && selected?.type === "productGroupTabs") return <div className="grid gap-3"><div className="flex items-center justify-between"><strong>选择商品分组</strong><button type="button" aria-label="关闭" onClick={()=>setShowGroupPicker(false)}><X size={18}/></button></div><ProductGroupTree categories={categories} groups={selectedGroups} onChange={(groups)=>update({groups})}/><div className="flex justify-end gap-2"><button className="btn" type="button" onClick={()=>setShowGroupPicker(false)}>取消</button><button className="btn btn-primary" type="button" onClick={()=>setShowGroupPicker(false)}>确认</button></div></div>;
