...
export function PageEditor({ page, publicUrl, store, employee, products, categories, pages }: Props) {
  ...
  function Properties() {
    if (showGroupPicker && selected?.type === "productGroupTabs") return <div className="grid gap-3"><div className="flex items-center justify-between"><strong>选择商品分组</strong><button type="button" aria-label="关闭" onClick={()=>setShowGroupPicker(false)}><X size={18}/></button></div><input className="field" placeholder="搜索分组" value={groupSearch} onChange={(event)=>{setGroupSearch(event.target.value);setGroupPage(0)}}/><div className="grid gap-2">{categoryPage.map(({root,children})=><div className="rounded border border-[#e6e1da] p-2" key={root.id}><label className="flex items-center gap-2 text-sm"><input type="checkbox" checked={selectedGroups.some((item)=>item.categoryId===root.id)} disabled={!selectedGroups.some((item)=>item.categoryId===root.id)&&selectedGroups.length>=15} onChange={(event)=>update({groups:event.target.checked?[...selectedGroups,{categoryId:root.id,limit:null}]:selectedGroups.filter((item)=>item.categoryId!==root.id)})}/><strong>{root.name}</strong><span className="muted ml-auto">{root.productCount}</span></label>{children.map((category)=><label className="ml-6 flex items-center gap-2 border-t border-[#eeeae5] py-2 text-sm" key={category.id}><input type="checkbox" checked={selectedGroups.some((item)=>item.categoryId===category.id)} disabled={!selectedGroups.some((item)=>item.categoryId===category.id)&&selectedGroups.length>=15} onChange={(event)=>update({groups:event.target.checked?[...selectedGroups,{categoryId:category.id,limit:null}]:selectedGroups.filter((item)=>item.categoryId!==category.id)})}/>{category.name}<span className="muted ml-auto">{category.productCount}</span></label>)}</div>)}</div><div className="flex justify-between"><button className="btn" type="button" disabled={!groupPage} onClick={()=>setGroupPage((value)=>value-1)}>上一页</button><button className="btn" type="button" disabled={(groupPage+1)*6>=matchingCategories.length} onClick={()=>setGroupPage((value)=>value+1)}>下一页</button></div><div className="flex justify-end gap-2"><button className="btn" type="button" onClick={()=>setShowGroupPicker(false)}>取消</button><button className="btn btn-primary" type="button" onClick={()=>setShowGroupPicker(false)}>确认</button></div></div>;
