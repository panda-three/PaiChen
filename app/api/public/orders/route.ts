...
export async function POST(request: Request) {
  ...
  const store=await db.store.findUnique({where:{slug:input.storeSlug}});if(!store?.isActive||!store.customerEnabled)return Response.json({error:"店铺暂不可用"},{status:404});const sourceEmployee=input.ref?await db.user.findFirst({where:{storeId:store.id,shareCode:input.ref,role:Role.EMPLOYEE,isActive:true}}):null;
