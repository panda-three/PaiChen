...
export function AdminShell({ actor, children }: { actor: User & { store: { name: string } | null }; children: React.ReactNode }) {
  return <div className="admin-app">
    ...
    <div className="admin-workspace">
      <header className="admin-topbar"><div><strong>{actor.store?.name ?? "平台总后台"}</strong><span>{actor.name}</span></div><form action={async () => { "use server"; await signOut({ redirectTo: "/login" }); }}><button className="admin-logout" title="退出登录" aria-label="退出登录"><LogOut size={17} /></button></form></header>
